...
class PostHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if 'message' in self.request.arguments:
            ...
            print '%s:MESSAGE to %s:%s' % (datetime.datetime.today().strftime("%d/%m/%y %H:%M:%S "), group, message)
            if hmac_key:
...
class DistributeHandler(tornado.websocket.WebSocketHandler):
    def open(self, params):
        ...
        print '%s:CONNECT to %s' % (datetime.datetime.today().strftime("%d/%m/%y %H:%M:%S "), self.group)
